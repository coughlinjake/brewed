...
module Brewed
  ...
  ##
  ## Stolen almost verbatim from https://github.com/granth/titleize
  ##
      ...
      if phrases.size > 1
        phrases[0..-2].each_with_index do |phrase, index|
          if SMALL_WORDS.include?(phrase.split.last.downcase)
            phrases[index] << SPACE + phrases.slice!(index + 1)
          end
        end
      end
...
if defined? ActiveSupport
  module ActiveSupport::Inflector
    ...
    def titleize(title, opts={})
      ...
      ::Brewed::Titleize.titleize(title)
    end
    alias_method :titlecase, :titleize
  end
end
